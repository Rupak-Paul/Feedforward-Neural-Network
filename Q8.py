...
class NuralNetwork:
    def __init__(self, numOfHiddenLayer, sizeOfHiddenLayer, inputSize, outputSize, activationFun, initilizationMethod, lossFunction):
    ...
    def trainByMomentumGradientDescent(self, epochs, batchSize, eta, beta, train_X, train_Y, val_X, val_Y, weightDecay):
        ...
        for i in range(epochs):
            ...
            trainLoss, trainAccuracy, validationLoss, validationAccuracy = self.__calculateAndPrintLossAndAccuracy(train_X, train_Y, val_X, val_Y)
            wandb.log({
                "epochs": i+1,
    ...
    def trainByNesterovGradientDescent(self, epochs, batchSize, eta, beta, train_X, train_Y, val_X, val_Y, weightDecay):
    ...
    def trainByStochasticGradientDescent(self, epochs, batchSize, eta, train_X, train_Y, val_X, val_Y, weightDecay):  
        for i in range(epochs):
            dw, db = self.__initilizeWandB()
            pointSeen = 0
    ...
    def trainByRmsprop(self, epochs, batchSize, eta, beta, train_X, train_Y, val_X, val_Y, weightDecay):
    ...
    def trainByAdam(self, epochs, batchSize, eta, beta1, beta2, train_X, train_Y, val_X, val_Y, weightDecay):
    ...
    def trainByNadam(self, epochs, batchSize, eta, beta1, beta2, train_X, train_Y, val_X, val_Y, weightDecay):
        m_w, m_b = self.__initilizeWandB()
        v_w, v_b = self.__initilizeWandB()
    ...
    def __backwardPropagation(self, A, H, input, output, weightDecay):
        ...
        delA = None
        if self.lossFunction == 'squared error':
            delA = -1.0 * (Y - YHat) * YHat * (1 - YHat)
        else:
            delA = -1.0 * (Y - YHat)
    ...
    def __calculateAndPrintLossAndAccuracy(self, train_X, train_Y, val_X, val_Y):
        trainLoss = self.__calculateAvgLoss(train_X, train_Y)
        trainAccuracy = self.__calculateAvgAccuracy(train_X, train_Y)
        validationLoss = self.__calculateAvgLoss(val_X, val_Y)
...
def main():
    sweep_config = {
        ...
        'parameters': {
            'epochs': {'values': [10]},
            'hiddenLayers': {'values': [1]},
            'hiddenLayerSize': {'values': [64]},
            'weightDecay': {'values': [0]},
            'learningRate': {'values': [1e-2]},
            'optimizer': {'values':['nadam']},
            'batchSize': {'values':[32]},
            'weightInitialisation': {'values':['Xavier']},
            'activationFunctions': {'values':['ReLU']},
            'lossFunction': {'values':['cross entropy', 'squared error']},
            'beta': {'values':[0.9]},
            'beta1': {'values':[0.9]},
            'beta2': {'values':[0.999]}
        }
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project='CS23M056_DL_Assignment_1')
    wandb.agent(sweep_id, function=callMe, count=2)
    wandb.finish()

def callMe():
    wandb.init(project='CS23M056_DL_Assignment_1')
    
    epochs = wandb.config['epochs']
    hiddenLayers = wandb.config['hiddenLayers']
    hiddenLayerSize = wandb.config['hiddenLayerSize']
    weightDecay = wandb.config['weightDecay']
    learningRate = wandb.config['learningRate']
    optimizer = wandb.config['optimizer']
    batchSize = wandb.config['batchSize']
    weightInitialisation = wandb.config['weightInitialisation']
    activationFunction = wandb.config['activationFunctions']
    lossFunction = wandb.config['lossFunction']
    ...
    wandb.run.name = 'bs-'+str(batchSize)+'-lr-'+ str(learningRate)+'-ep-'+str(epochs)+ '-op-'+str(optimizer)+ '-nhl-'+str(hiddenLayers)+'-shl-'+str(hiddenLayerSize)+ '-act-'+str(activationFunction)+'-wd-'+str(weightDecay)+'-wi-'+str(weightInitialisation)
    
    NL = NuralNetwork(hiddenLayers, hiddenLayerSize, inputSize, outputSize, activationFunction, weightInitialisation, lossFunction)
    if(optimizer == "sgd"):
        NL.trainByStochasticGradientDescent(epochs, batchSize, learningRate, train_images, train_labels, val_images, val_labels, weightDecay)
    elif(optimizer == "momentum"):
        NL.trainByMomentumGradientDescent(epochs, batchSize, learningRate, beta, train_images, train_labels, val_images, val_labels, weightDecay)
    elif(optimizer == "nesterov"):
        NL.trainByNesterovGradientDescent(epochs, batchSize, learningRate, beta, train_images, train_labels, val_images, val_labels, weightDecay)
    elif(optimizer == "rmsprop"):
        NL.trainByRmsprop(epochs, batchSize, learningRate, beta, train_images, train_labels, val_images, val_labels, weightDecay)
    elif(optimizer == "adam"):
        NL.trainByAdam(epochs, batchSize, learningRate, beta1, beta2, train_images, train_labels, val_images, val_labels, weightDecay)
    elif(optimizer == "nadam"):
        NL.trainByNadam(epochs, batchSize, learningRate, beta1, beta2, train_images, train_labels, val_images, val_labels, weightDecay)
